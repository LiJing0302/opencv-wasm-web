import { wasmFeatureDetect } from "@/utils/wasmFeatureDetect";
...
export const loadOpenCV = async (
  ...
  const simdSupported = wasmSupported ? await wasmFeatureDetect.simd() : false;

  const threadsSupported = wasmSupported
    ? await wasmFeatureDetect.threads()
    : false;
  Message.error(
    `wasmSupported:${simdSupported}，simdSupported:${simdSupported}，threadsSupported:${threadsSupported}`
  );
